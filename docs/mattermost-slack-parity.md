...
| `slash_command_entrypoint` | interaction | direct_mattermost_feature | Mattermost-native `/orka` slash commands validate the command token, re-check bot and sender channel membership, and route `ask`/`sources` through the durable Onyx handler while `help`/`status` stay ephemeral. | `backend/onyx/onyxbot/mattermost/commands.py:handle_mattermost_slash_command`; `backend/onyx/onyxbot/mattermost/run.py:_handle_slash_command_request`; durable event ledger |
