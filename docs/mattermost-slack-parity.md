...
| --- | --- | --- | --- | --- |
| `bot_instance_configuration` | bot | policy_difference | Deployment-file Mattermost service config for the release PoC; later dashboard CRUD is tracked by issue #31. | `backend/onyx/onyxbot/mattermost/config.py:MattermostBotConfig`; fallback: Operators configure the disposable test bot outside Onyx until Mattermost dashboard CRUD ships. |
...
| `slash_command_entrypoint` | interaction | platform_gap | Mattermost-native slash command UX is feasible but not shipped in the issue #30 release artifact. | fallback: Use DM, mention, or root allowlisted post until issue #33 ships Mattermost slash commands. |
