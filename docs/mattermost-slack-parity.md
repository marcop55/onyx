...
| `ephemeral_or_private_answer` | interaction | platform_gap | Mattermost ephemeral posts are a native equivalent, but this release answers visibly in-thread to avoid identity leaks. | fallback: Use visible thread replies until issue #34 ships explicit ephemeral response handling. |
