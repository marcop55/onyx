...
MATTERMOST_SLACK_PARITY_MATRIX: tuple[SlackToMattermostCapability, ...] = (
    SlackToMattermostCapability(
        ...
        mattermost_contract="Onyx managed DB/API/admin UI Mattermost bot configuration stores encrypted tokens, enablement, identity, health state, and slash-command credentials for steady-state operation.",
    ...
    SlackToMattermostCapability(
        ...
        mattermost_contract="Managed Mattermost channel configs select the Onyx Agent/persona per bot channel, falling back through bot default to the safe configured persona while preserving the shared PoC knowledge scope for admitted users and leaving base instructions owned by the selected Agent.",
        status=MattermostParityStatus.DIRECT_MATTERMOST_FEATURE,
        evidence=(
            "backend/onyx/server/manage/slack_bot.py:create_slack_channel_config",
            "backend/onyx/server/manage/models.py:SlackChannelConfigCreationRequest",
            "backend/onyx/db/models.py:SlackChannelConfig",
            "backend/onyx/db/models.py:MattermostChannelConfig",
            "backend/onyx/db/mattermost_bot.py:fetch_mattermost_channel_config_for_bot_and_channel",
            "backend/onyx/server/manage/mattermost_bot.py:create_mattermost_channel_config",
            "backend/onyx/onyxbot/mattermost/handler.py:MattermostHandlerConfig",
            "web/src/app/admin/mattermost-bots/MattermostChannelConfigPanel.tsx:MattermostChannelConfigPanel",
            "backend/tests/unit/onyx/onyxbot/mattermost/test_channel_config.py:test_success_resolves_channel_override_agent_and_response_style",
        ),
        guarantees=frozenset(
            {
                "full_admin_panel_access_required",
                "agent_instructions_owned_by_onyx",
                "managed_credentials",
                "provider_neutral",
                "shared_full_corpus",
                "steady_state_managed_config",
    ...
    SlackToMattermostCapability(
        ...
        evidence=(
            ...
            "backend/onyx/server/manage/models.py:MattermostResponseStyle",
    ...
    SlackToMattermostCapability(
        ...
        mattermost_contract="Mattermost-native ephemeral posts deliver slash-command and managed configured private-answer channels only to the originating sender, with durable delivery mode and terminal outcome checkpoints so replay cannot promote them to public posts or rerun the model.",
        ...
        evidence=(
            ...
            "backend/onyx/onyxbot/mattermost/run.py:_build_handler_config",
            "backend/onyx/db/models.py:MattermostChannelConfig",
            "backend/onyx/db/mattermost_bot.py:fetch_mattermost_private_answer_channel_ids",
            "backend/onyx/server/manage/mattermost_bot.py:create_mattermost_channel_config",
            "web/src/app/admin/mattermost-bots/MattermostBotForm.tsx:Private answer channels",
            ...
            "backend/tests/unit/onyx/onyxbot/mattermost/test_run.py:test_slash_command_runtime_uses_managed_private_answer_channels",
            *_MEMBERSHIP_EVIDENCE,
            *_LEDGER_EVIDENCE,
        ),
        guarantees=frozenset(
    ...
    SlackToMattermostCapability(
        ...
        mattermost_contract="Mattermost-native signed answer action buttons are attached to final answer posts; post edits against adapter-owned roots remain replay-safe retry targets.",
        status=MattermostParityStatus.MATTERMOST_NATIVE_EQUIVALENT,
        evidence=(
            "backend/onyx/onyxbot/mattermost/interactive.py:build_mattermost_answer_action_props",
            "backend/onyx/onyxbot/mattermost/streaming.py:MattermostFinalPropsFactory",
            "backend/onyx/onyxbot/mattermost/handler.py:_build_final_action_props_factory",
            "backend/tests/unit/onyx/onyxbot/mattermost/test_interactive_actions.py",
            *_CHAT_EVIDENCE,
        ),
        guarantees=_CHAT_GUARANTEES,
    ...
    SlackToMattermostCapability(
        ...
        mattermost_contract="Mattermost reactions and signed native answer buttons on owned answer posts map to Onyx chat-message feedback with Mattermost attribution.",
        status=MattermostParityStatus.DIRECT_MATTERMOST_FEATURE,
        evidence=(
            "backend/onyx/onyxbot/mattermost/interactive.py:handle_mattermost_interactive_action",
    ...
    SlackToMattermostCapability(
        ...
        mattermost_contract="Mattermost-native signed answer buttons let admitted users mark answers as needing follow-up or resolved, with durable feedback records and replay-safe confirmations.",
        status=MattermostParityStatus.MATTERMOST_NATIVE_EQUIVALENT,
        evidence=(
            "backend/onyx/onyxbot/mattermost/interactive.py:handle_mattermost_interactive_action",
            "backend/onyx/db/mattermost_bot.py:complete_mattermost_interactive_feedback_event",
            "backend/tests/unit/onyx/onyxbot/mattermost/test_interactive_actions.py",
            "backend/onyx/onyxbot/slack/handlers/handle_buttons.py:handle_followup_button",
            *_MEMBERSHIP_EVIDENCE,
        ),
        guarantees=frozenset({"membership_fail_closed", "replay_safe"}),
    ),
    SlackToMattermostCapability(
    ...
    SlackToMattermostCapability(
        ...
        mattermost_contract="Mattermost file metadata is ownership/type/size-validated before download; accepted bytes are checksummed and recorded with stable user-file IDs once for event attachments.",
        status=MattermostParityStatus.DIRECT_MATTERMOST_FEATURE,
        evidence=(
            "backend/onyx/onyxbot/mattermost/client.py:get_file_info",
            "backend/onyx/onyxbot/mattermost/attachments.py:save_mattermost_attachments",
            ...
            "backend/tests/unit/onyx/onyxbot/mattermost/test_attachments.py",
        ),
        guarantees=frozenset({"replay_safe", "shared_full_corpus"}),
    ),
    SlackToMattermostCapability(
        ...
        evidence=(
            ...
            "backend/onyx/onyxbot/mattermost/run.py:_handle_interactive_action_request",
            "backend/tests/unit/onyx/onyxbot/mattermost/test_interactive_actions.py",
        ),
